...
Definition exec_mon_introspect (kstate: kstate_t) (owner i j: nat) : option (int64 * int64) :=
  ...
  | Some ci =>
    if decide (i ≤ j ∧ j < i + ci.(cfree)) then
      ...
      | None => None
      | Some cj => 
        let wowner := match cj.(cowner) with None => Int64.zero | Some v => n_to_int64 (S v) end in
        let wfree := (n_to_int64 cj.(cfree)) <<₆₄ (Int64.repr 16) in
        let wsize := (n_to_int64 cj.(csize)) <<₆₄ (Int64.repr 32) in
        let wpid  := (n_to_int64 cj.(cdata).(mpid)) <<₆₄ (Int64.repr 48) in
  ...
  | Some ci =>
    if decide (ci.(cfree) = ci.(csize)) then
      ...
      | Some cj =>
        ...
        let ci' := ci <| (@cfree tsl_t) := ci.(cfree) + cj.(cfree) |> <| (@cdata tsl_t) := di' |> in
        let ktsl_tbl' := cap_set (cap_set kstate.(ktsl_tbl) i (Some ci')) j None in
        let ksched' := if decide (cj.(cdata).(tfree) ≠ 0) then
                         sched_set (sched_set kstate.(ksched) cj.(cdata).(thart) cj.(cdata).(tbase) None 0)
  ...
  | Some ci =>
      ...
      | None =>
          ...
          then
            ...
            let kptable' := ptable_pmp_try_set kstate.(kptable) ci.(cowner) (Some pmpreg) (Some pmpconf) in
